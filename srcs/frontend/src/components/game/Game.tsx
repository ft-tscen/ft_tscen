...
function Game({
	...
	useEffect(() => {
		try {
			...
			myGameSocket.socket.on("end-game", (p1win: boolean) => {
				...
				if (ctx) {
					if (mod === gameMod.soloGame) {
						if (p1win) EndPage(ctx, CanvasWidth, CanvasHeight, "user");
						else EndPage(ctx, CanvasWidth, CanvasHeight, "com");
					} else {
						if (p1win) EndPage(ctx, CanvasWidth, CanvasHeight, "p1");
						else EndPage(ctx, CanvasWidth, CanvasHeight, "p2");
					}
					// 여기에 경기 결과 db에 업데이트 하는 코드 추가 (watch상태 아닐시에만)
					if (mod === gameMod.rankGame) {
						if (!isChangedGameData) setChangedGameData(true);
						// socket emit
					}
	...
	return (
		<>
			{isInfo && <Player mod={mod} playerInfo={playerInfo}></Player>}
