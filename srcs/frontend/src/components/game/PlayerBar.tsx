	...
	useEffect(() => {
		if (mod !== gameMod.soloGame) {
			const getLPlayer = async () => {
				...
				let wins: number = 0;
				let losses: number = 0;
				if (playerInfo?.p1.nickname) name = playerInfo?.p1.nickname;
				else name = "user";
				if (playerInfo?.p1.avatarId) {
					await api
						.get(`/user/avatar/${playerInfo?.p1.avatarId}`, {
				...
				} else setLeftUrl("https://www.w3schools.com/howto/img_avatar.png");
				try {
					const res = await api.get(
						`/user/score?nickname=${playerInfo?.p1.nickname}`
					);
			...
			const getRPlayer = async () => {
				...
				let wins: number = 0;
				let losses: number = 0;
				if (playerInfo?.p2.nickname) name = playerInfo?.p2.nickname;
				else name = "user";
				if (playerInfo?.p2.avatarId) {
					await api
						.get(`/user/avatar/${playerInfo?.p2.avatarId}`, {
							responseType: "arraybuffer",
						})
						.then((response) => {
							const arrayBufferView = new Uint8Array(response.data);
						...
						.catch((error) => console.error(error));
				} else setRightUrl("https://www.w3schools.com/howto/img_avatar.png");
				try {
					const res = await api.get(
						`/user/score?nickname=${playerInfo?.p2.nickname}`
					);
					if (mod === gameMod.rankGame) {
						wins = res.data?.score.r_win;
						losses = res.data?.score.r_lose;
